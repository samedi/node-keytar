...
void FindCredentialsWorker::HandleOKCallback() {
  ...
  if (success) {
    ...
    unsigned int idx = 0;
    for (std::vector<keytar::Credentials>::iterator it = credentials.begin(); it != credentials.end(); it++) {
      keytar::Credentials cred = *it;
      v8::Local<v8::Object> obj = Nan::New<v8::Object>();

      v8::Local<v8::String> account = Nan::New<v8::String>(cred.first.data(), cred.first.length()).ToLocalChecked();
      v8::Local<v8::String> password = Nan::New<v8::String>(cred.second.data(), cred.second.length()).ToLocalChecked();
